	...
	<form  autocomplete="off" method="get" action="/find-my-clinic">
		<div class="form-block border">
			...
			<select name="body_part_id" class="border" id="body-part-select" >
				...
				<option value="{{$data->id}}" @if($body_part_id ==$data->id ) selected="selected" @endif>{{$data->name_en}}</option>
				@endforeach					
			</select>
		</div>
		<div class="form-block border">
			...
			<select name="procedure_info_id"  class="border" id="procedure-info-select">
				...
				<option value="{{$data->id}}" @if($procedure_info_id ==$data->id ) selected="selected" @endif>{{$data->name_en}}</option>
				@endforeach		
			</select>
		</div>
		<div class="form-block border">
			...
			<select id="property-type" class="border" name="area">
				...
				<option value="{{$data->group_value}}" @if($area == $data->group_value) selected="selected" @endif>{{$data->group_str}}</option>
				@endforeach
			</select>
		</div>
		<div class="form-block border">
			...
			<select id="property-type" class="border" name="medical_id">
				...
				<option value="{{$data->id}}" @if($medical_id ==$data->id ) selected="selected" @endif>{{$data->name_en}}</option>
				@endforeach	
			</select>
		</div>	
				...
				<form  autocomplete="off" method="get" action="/find-my-clinic">
					...
					<div class="form-block border">
						...
						<select id="property-type" class="border" name="area">
							<option value="">Any</option>
							@foreach($areaData as $data)
							<option value="{{$data->group_value}}">{{$data->group_str}}</option>
							@endforeach
						</select>
					</div>
					<div class="form-block border">
